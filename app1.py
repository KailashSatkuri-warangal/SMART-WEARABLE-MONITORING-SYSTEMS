...
def evaluate_model():
    ...
    plt.scatter(y[:1000], y_pred[:1000], color='blue', alpha=0.5, label='Predicted vs Actual')
    plt.plot([y.min(), y.max()], [y.min(), y.max()], 'r--', lw=2)
    plt.xlabel('Actual Calories')
    plt.ylabel('Predicted Calories')
    plt.title(f'Actual vs Predicted (MSE: {mse:.2f}, R²: {r2:.2f})')
    plt.grid(True)
    plt.savefig('static/actual_vs_predicted.png')
    plt.close()

    print(f"Model evaluated at {datetime.now()}")
